      ...
      <div class="message" v-for="(item, index) in userGoods" :key="index">
        ...
        <div class="info">
          ...
          <div class="published-message-operation">
            ...
            <span class="down" v-if="$store.getters.isAdmin&&item.orderStatus==0&&item.type!='needs'" @click="takeDown(item.orderId)">离校</span>
            <span class="down" v-if="$store.getters.isAdmin&&item.orderStatus==1&&item.type!='needs'" @click="takeUp(item.orderId)">登记信息</span>
          </div>
        </div>
      </div>
      <Pagination
